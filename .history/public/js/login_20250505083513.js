...
async function handleGoogleAuth(mode) {
    try {
        ...
        provider.setCustomParameters({
            ...
            login_hint: mode === 'login' ? 'user@example.com' : '',
        });
        
        if (DEBUG) console.log('Starting Google authentication with popup...');
        
        try {
            // First try redirect method if on mobile
            const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
            
            if (isMobile) {
                if (DEBUG) console.log('Mobile device detected, using redirect method');
                // On mobile devices, use redirect for better experience
                ...
                if (DEBUG) console.log('Google auth successful:', user.email);
                showAlert(`Welcome ${user.displayName || 'back'}!`, 'success');
            ...
            if (DEBUG) console.error('Popup error:', popupError.code, popupError.message);
            
            // Handle specific popup errors
            if (popupError.code === 'auth/popup-blocked') {
                showAlert('Popup blocked by browser. Trying redirect instead...', 'warning');
                // Fall back to redirect auth if popup is blocked
                await auth.signInWithRedirect(provider);
                return;
                ...
                showAlert('Domain not authorized. Please contact administrator.', 'error');
                
                if (DEBUG) {
                    console.error('IMPORTANT: Your domain is not authorized in Firebase.');
                    console.error('Go to Firebase Console > Authentication > Settings > Authorized domains');
                    console.error('And add your domain (including localhost for testing)');
                }
                return;
            }
        ...
        if (DEBUG) console.error('Google auth general error:', error);
        
        // Handle specific error cases
        if (error.code === 'auth/network-request-failed') {
            showAlert('Network error. Please check your internet connection.', 'error');
        } else if (error.code === 'auth/operation-not-allowed') {
            showAlert('Google sign-in is not enabled. Please contact administrator.', 'error');
            
            if (DEBUG) {
                console.error('IMPORTANT: Google Authentication is not enabled in Firebase.');
                console.error('Go to Firebase Console > Authentication > Sign-in method');
                console.error('And enable Google as a sign-in provider');
            }
        } else {
